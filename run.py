...
def init_connections():
    ...
    # API.AI
    apiai_token = os.environ.get("APIAI_CLIENT_ACCESS_TOKEN_2", os.getenv("APIAI_CLIENT_ACCESS_TOKEN_F"))
    ...
    if not all((conversation_username, conversation_password, workspace_id, bot_id, slack_bot_token, nlu_username, nlu_password, cortical_key, gv_bot_deafault_channel_name, apiai_token)):
...
#                 gl_bot_deafault_channel_name = gv_bot_deafault_channel_name or conversation_creds['bot_deafault_channel']
            ...
            if not all((conversation_username, conversation_password, workspace_id, bot_id, bot_name, slack_bot_token, nlu_username, nlu_password, cortical_key, gv_bot_deafault_channel_name, apiai_token)):
...
def main():
    ...
    if slack_client.rtm_connect():
        ...
        while True: 
            slack_output = slack_client.rtm_read()                                              # get output from Slack
            if slack_output:
                # LOG.debug("slack output\n:{}\n".format(slack_output))
                print("Slack Output: ", slack_output)
                message, channel, user = parse_slack_output(bot_id, bot_name, slack_output)     # parse Slack output to extract message text, channel and user
                print("Slack message: ", message)
                if message and channel and message != None and channel != None:                 # if message text and channel was extracted from the Slack output, pass it to API.AI
                    request = gv_ai.text_request()
                    request.query = message                                                     # send message to API.AI
                    if len(message) < 256:
                        response = json.loads(request.getresponse().read().decode('utf-8'))         # get output from API.AI
                        print("AI Output: ", response)
                        result = response['result']
                        result_text = response['result']['fulfillment']['speech']                   # extract text message from the output of API.AI
                        print("AI message: ", result_text)                                          # extract action from the output of API.AI
                        action = result.get('action')
                        actionIncomplete = result.get('actionIncomplete', False)
                        vParameter = result.get("parameters").get("articleurl")
                    
                        if action == 'article.analyze' and vParameter!='' and vParameter!=None :                                          # checking if API.AI action is to analyze an article 
                            print("An action: Analyze Article")
                            result_text = handle_message(conversation_client, slack_client, workspace_id, context, message, channel, user)
                    else:
                        result_text = "All queries should be less than 256 symbols"
...
#                  slack_client.api_call("chat.postMessage", channel=gv_bot_deafault_channel_id, text=response, as_user=True)      ## uncomment to post responses at Default channel
                    slack_client.api_call("chat.postMessage", channel=channel, text=result_text, as_user=True)                        ## uncomment to post responses at the sender's channel
